...
class NBAcard:
    ...
    def _get_card_price(self, start_year, end_year, keywords):
        ...
        for year in range(start_year, end_year, 1):
            text = requests.get(
                f"https://www.pwccmarketplace.com/market-price-research?q={keywords}&year_min={year}&year_max={year+1}&items_per_page=100",
                headers=headers,
            ).text
    ...
    def _get_player_suffix(self, name):
        normalized_name = unidecode.unidecode(
            unicodedata.normalize("NFD", self.name)
            .encode("ascii", "ignore")
            .decode("utf-8")
        )
        initial = normalized_name.split(" ")[1][0].lower()
        suffix = (
            "/players/" + initial + "/" + self._create_suffix(self.name) + "01.html"
        )
    ...
    def _create_suffix(self, name):
        normalized_name = (
            unicodedata.normalize("NFD", self.name.replace(".", ""))
            .encode("ascii", "ignore")
            .decode("utf-8")
        )
    ...
    def make_player_df(self):
        ...
        df["card_price"] = df["Season"].apply(
            lambda x: year_price[int(x.split("-")[0])]
        )
        df["fantasy_p"] = round(
            df["PTS"]
            + (df["TRB"] * 1.2)
    ...
    def draw(self, df):
        ...
        fig = (
            df[["player_info", "PTS", "fantasy_p"]]
            .plot.bar(x="player_info", figsize=(28.5, 20.5), rot="290")
            .get_figure()
        )
